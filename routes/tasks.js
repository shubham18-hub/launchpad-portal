...
router.get('/', requireAuth, async (req, res) => {
  ...
  if (['admin', 'reviewer'].includes(req.user.role)) {
    const { rows: countRows } = await pool.query(
      'SELECT task_id, COUNT(*)::int AS count FROM submissions GROUP BY task_id'
    );
    counts = Object.fromEntries(countRows.map((c) => [c.task_id, c.count]));
  }

  res.json(
    tasks.map((t) => ({
      ...t,
      submission: byTask[t.id] || null,
      submission_count: counts[t.id] ?? null,
    }))
  );
});
