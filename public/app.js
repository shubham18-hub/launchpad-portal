...
function switchAuthTab(tab) {
  ...
  $('#authTabSignin').hidden = tab !== 'signin';
  $('#authTabSignup').hidden = tab !== 'signup';
}
...
$('#registerForm').addEventListener('submit', async (e) => {
  ...
  const data = Object.fromEntries(new FormData(e.target));
  try {
    currentUser = await api('/auth/register', { method: 'POST', body: JSON.stringify(data) });
    updateNav();
    await loadTasks();
    closeModals();
    e.target.reset();
    toast(`Welcome to Launchpad, ${currentUser.name}.`);
  } catch (err) {
    toast(err.message);
  }
});
